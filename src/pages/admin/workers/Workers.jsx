...
const RuberoidFactoryHR = () => {
    ...
    const { data: employees = [], isLoading, isError, error } = useGetWorkersQuery();
    ...
    const handleAddEmployee = async (newEmployee) => {
        try {
            ...
            toast.error(`Ishchi qo'shishda xatolik: ${err.data?.message || err.message}`);
        }
    };
    ...
    const handleUpdateEmployee = async (updatedEmployee) => {
        try {
            ...
            toast.error(`Ma'lumotlarni yangilashda xatolik: ${err.data?.message || err.message}`);
        }
    };
                        ...
                        <button
                            onClick={async () => {
                                ...
                                try {
                                    ...
                                    toast.error(`O'chirishda xatolik: ${err.data?.message || err.message}`);
                                    closeToast();
                                }
                            }}
                    ...
                    <div className="header-stats-panel">
                        <div className="stat-item-card">
                            ...
                            <div className="stat-details">
                                <span className="stat-number">{employees?.innerData.length}</span>
                        ...
                        <div className="stat-item-card">
                            ...
                            <div className="stat-details">
                                <span className="stat-number">{employees?.innerData?.filter((e) => e.isOfficeWorker).length}</span>
                        ...
                        <tbody className="table-body-section">
                            {filteredEmployees?.innerData?.map((employee, inx) => (
                                <tr key={inx} className="employee-table-row">
                                    ...
                                    <td className="salary-amount-cell">{formatSalary(employee.salary, employee.paymentType)}</td>
                                    <td className="login-credentials-cell">
                                        {employee.isOfficeWorker ? (
                                            <div className="credentials-info">
                                                ...
                                                {employee.role && <span className="role-badge">{roles[employee.role]}</span>}
                                            </div>
                                        ) : (
                                            <span className="no-credentials-text">Login talab qilinmaydi</span>
                                        )}
                                    </td>
