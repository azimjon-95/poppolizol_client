...
const SalaryManagement = () => {
    ...
    const {
        ...
        isError,
        error,
    } = useGetAllEmployeesSalaryInfoQuery({ month: selectedMonth, year: selectedYear });
    ...
    useEffect(() => {
        ...
        const filtered = employees.filter((emp) => {
            ...
            return fullName.includes(search) || department.includes(search) || position.includes(search);
        });
    ...
    const handlePenalty = async (e) => {
        ...
        try {
            ...
            showAlert('Jarima qo\'shishda xatolik yuz berdi: ' + error.message, 'error');
        }
    };
    ...
    const getStatusBadge = (status) => {
        const statusConfig = {
            "to'liq_to'langan": { class: 'factory-status-paid', text: "To'liq to'langan", icon: '✓' },
            "to'liq_to'lanmagan": { class: 'factory-status-partial', text: "To'liq to'lanmagan", icon: '⏳' },
            "ortiqcha_to'langan": { class: 'factory-status-overpaid', text: "Ortiqcha to'langan", icon: '↑' },
        };
    ...
    const calculateSummary = () => {
        const totalSalary = filteredEmployees.reduce((sum, emp) => sum + emp.salaryPayment.baseSalary, 0);
        const totalPaid = filteredEmployees.reduce((sum, emp) => sum + emp.salaryPayment.totalPaid, 0);
        const totalPenalties = filteredEmployees.reduce((sum, emp) => sum + emp.salaryPayment.penaltyAmount, 0);
        const totalRemaining = filteredEmployees.reduce((sum, emp) => sum + emp.salaryPayment.remainingAmount, 0);
            ...
            <div className="factory-employees-table-container">
                <div className="factory-table-header">
                    <h2 className="factory-table-title">
                        👥 Ishchilar ro'yxati ({months.find((m) => m.value === selectedMonth)?.label} {selectedYear})
                    </h2>
                ...
                {isLoading ? (
                    ...
                    <div className="factory-empty-state">
                        ...
                        <p className="factory-empty-state-description">{error?.message || 'Ma\'lumotlarni yuklashda xatolik'}</p>
                    </div>
                    ...
                    <div className="factory-empty-state">
                        ...
                        <p className="factory-empty-state-description">
                            {searchQuery ? "Qidiruv bo'yicha natija topilmadi" : "Bu oy uchun ishchilar ma'lumoti yo'q"}
                        </p>
                    </div>
                ) : (
                                ...
                                {filteredEmployees.map((employee) => (
                                    <tr key={employee._id}>
                                        <td>
                                            ...
                                            <div className="factory-employee-position">{employee.position}</div>
                                        </td>
                                        <td>
                                        ...
                                        <td>
                                            <span
                                                className={`factory-amount-display ${employee.salaryPayment.totalPaid > 0 ? 'factory-amount-positive' : 'factory-amount-zero'}`}
                                            >
                                        ...
                                        <td>
                                            <span
                                                className={`factory-amount-display ${employee.salaryPayment.penaltyAmount > 0 ? 'factory-amount-negative' : 'factory-amount-zero'}`}
                                            >
                                        ...
                                        <td>
                                            <span
                                                className={`factory-amount-display ${employee.salaryPayment.advanceAmount > 0 ? 'factory-amount-positive' : 'factory-amount-zero'}`}
                                            >
                    ...
                    <div className="factory-modal">
                        <div className="factory-modal-header">
                            <h2 className="factory-modal-title">
                                💳 To'lov qilish - {selectedEmployee?.firstName} {selectedEmployee?.lastName}
                            </h2>
                            <button className="factory-modal-close" onClick={() => setShowPaymentModal(false)}>
                                ×
                            </button>
                        </div>
                        <form onSubmit={handlePayment}>
                                    ...
                                    <div className="factory-payment-method-grid">
                                        {Object.entries(paymentMethods).map(([key, value]) => (
                                            <div className="factory-payment-method-option" key={key}>
                                                ...
                                                />
                                                <label htmlFor={key} className="factory-payment-method-label">
                                                    {value}
                                                </label>
                                            </div>
                                        ))}
                            ...
                            <div className="factory-modal-footer">
                                ...
                                <button type="submit" className="factory-btn factory-btn-primary" disabled={isLoading}>
                                    {isLoading ? <span className="factory-loading-spinner"></span> : "To'lovni tasdiqlash"}
                                </button>
                            </div>
                        </form>
                    </div>
            ...
            {showPenaltyModal && (
                <div className="factory-modal-overlay">
                    <div className="factory-modal">
                        <div className="factory-modal-header">
                            <h2 className="factory-modal-title">
                                ⚠️ Jarima qo'shish - {selectedEmployee?.firstName} {selectedEmployee?.lastName}
                            </h2>
                            <button className="factory-modal-close" onClick={() => setShowPenaltyModal(false)}>
                                ×
                            </button>
                        </div>
                        <form onSubmit={handlePenalty}>
                            ...
                            <div className="factory-modal-footer">
                                ...
                                <button type="submit" className="factory-btn factory-btn-warning" disabled={isLoading}>
                                    {isLoading ? <span className="factory-loading-spinner"></span> : "Jarimani tasdiqlash"}
                                </button>
                            </div>
                        </form>
                    </div>
