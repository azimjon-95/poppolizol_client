...
import { useGetFilteredMaterialsQuery, useUpdateMaterialMutation, useCreateMaterialMutation } from '../../../context/materialApi';
import './style.css';
...
const BitumProductionSystem = () => {
    ...
    const materialObj = material?.reduce((acc, item) => {
    ...
    const workers = [
        { fullName: "Akmal Qurbonov", salary: 5200000 },
        { fullName: "Zohid Karimov", salary: 4000000 },
        { fullName: "Bilol Azizov", salary: 3300000 },
        // { fullName: "Dilmurod Jalilov", salary: 4500000 },
    ];
    // Narxlar
    const electricityPrice = 450;
    const gasPrice = 1800;
    // Hisoblash
    useEffect(() => {
        const { bn3Amount, wasteAmount, gasAmount, electricity } = currentBn3Production;
        ...
        const totalWorkersSalary = workers.reduce((sum, worker) => sum + worker.salary, 0);
        ...
        const totalCost = totalBn3Cost + totalGasCost + totalElectricityCost + totalWorkersSalary;
    ...
    const handleBn3Production = async () => {
        ...
        if (!gas || hours !== 19) return alert('Gaz miqdorini kiriting va 19 soat qaynaganligini tasdiqlang!');
        ...
        materialObj?.bn5
            ? await updateMaterial({ id: materialObj.bn5._id, body: { ...materialObj.bn5, quantity: materialObj.bn5.quantity + finalBn5 } })
    ...
    const confirmBn3Production = () => {
        ...
        setBn3ToBn5Productions([...bn3ToBn5Productions, {
        ...
        setCurrentBn3Production({ ...currentBn3Production, id: null, bn3Amount: '15000', wasteAmount: '500', gasAmount: '4500', notes: '', electricity: '1200', price: '' });
        setActualBn5Result('');
    ...
    const handleBn5Processing = () => {
        ...
        if (Math.abs(bn5Percent - 75) > 5 || Math.abs(melPercent - 25) > 5)
            return alert(`Nisbat noto'g'ri! BN-5: ${bn5Percent.toFixed(1)}%, Mel: ${melPercent.toFixed(1)}%`);
    ...
    const confirmBn5Processing = () => {
        ...
        if (inventory.exportBagStock < exportBagsNum) return alert('Yetarli eksport qopi yo\'q!');
        if (inventory.ropeStock < ropeNeeded) return alert('Yetarli ip yo\'q!');
        if (inventory.kraftPaperStock < kraftNeeded) return alert('Yetarli kraft qog\'oz yo\'q');
        if (totalPacked > totalMix) return alert('Qadoqlash miqdori aralashmadan ko\'p bo\'lishi mumkin emas!');

        setBn5Processing([...bn5Processing, {
            ...currentBn5Process,
            id: Date.now(),
            totalMix,
        ...
        setCurrentBn5Process({ ...currentBn5Process, id: null, bn5Amount: '5000', melAmount: '1800', exportBags: '', localBags: '', notes: '' });
        ...
        <div className="bitum-system-container">
            <div className="bitum-inventory-grid">
                ...
                <div className="bitum-inventory-card">
                    ...
                    <p className="ready-product-inventory">
                        Eksport: {inventory.readyProductStorage.exportBags} qop<br />
            ...
            <div className="bitum-production-sections">
                <div className="bitum-production-panel">
                    ...
                    <div className="bitum-input-grid">
                        {[
                            ...
                            { label: 'BN-5 (1kg narx)', key: 'price', placeholder: '5.850', readOnly: true },
                ...
                <div className="bitum-production-panel">
                    ...
                    <button className="bitum-action-button bitum-bn5-action" onClick={handleBn5Processing}>
                ...
                <div className="bitum-dialog-overlay">
                    <div className="bitum-dialog-box">
                        ...
                        <div className="bitum-dialog-actions">
                            <button className="bitum-cancel-button" onClick={() => setShowBn3Dialog(false)}>Bekor qilish</button>
                            <button className="bitum-confirm-button bitum-bn3-confirm" onClick={confirmBn3Production}>Tasdiqlash</button>
                        </div>
                    </div>
                </div>
            )}
            ...
            {showBn5ProcessDialog && (
                <div className="bitum-dialog-overlay">
                    <div className="bitum-dialog-box">
                        ...
                        <p>Umumiy aralashma: <strong>{(parseFloat(currentBn5Process.bn5Amount) + parseFloat(currentBn5Process.melAmount)).toLocaleString()} kg</strong></p>
                        <div className="bitum-input-grid">
                            {[
                                { label: 'Eksport qoplari (39 kg)', key: 'exportBags', placeholder: '0' },
                                { label: 'Ichki bozor qoplari (35 kg)', key: 'localBags', placeholder: '0' },
                            ].map(({ label, key, placeholder }) => (
                                <div className="bitum-input-group" key={key}>
                                    <label>{label}</label>
                                    <input
                        ...
                        <div className="bitum-packaging-details">
                            <p>Eksport miqdori: <strong>{((parseFloat(currentBn5Process.exportBags) || 0) * recipe.exportBagWeight).toLocaleString()} kg</strong></p>
                            <p>Ichki bozor miqdori: <strong>{((parseFloat(currentBn5Process.localBags) || 0) * recipe.localBagWeight).toLocaleString()} kg</strong></p>
                            <p>Ip sarfi: <strong>{((parseFloat(currentBn5Process.exportBags) || 0) * recipe.ropePerExportBag).toLocaleString()} metr</strong></p>
                            <p>Kraft qog'oz sarfi: <strong>{((parseFloat(currentBn5Process.localBags) || 0) * recipe.kraftPaperPerBag).toLocaleString()} kg</strong></p>
                        </div>
                        <div className="bitum-dialog-actions">
                            <button className="bitum-cancel-button" onClick={() => setShowBn5ProcessDialog(false)}>Bekor qilish</button>
                            <button className="bitum-confirm-button bitum-bn5-confirm" onClick={confirmBn5Processing}>Tasdiqlash</button>
                        </div>
                    </div>
                </div>
            )}
