...
const Bn5ProcessDialog = ({ refetch, material, gasPrice, electricityPrice }) => {
    ...
    useEffect(() => {
        ...
        const total = bn5Sum + melSum + gazSum + elektrSum + qopSum + kraftSum + ishHaqi;

        const extraForCalculation = (parseFloat(currentBn5Process.extra) || 271000) * 0.01;
    ...
    }, [currentBn5Process.bn5Amount, currentBn5Process.melAmount, currentBn5Process.electricity, currentBn5Process.gasAmount, currentBn5Process.kraftPaper, currentBn5Process.qop, material]);
    ...
    const validateBn5Processing = () => {
        ...
        if (
            ...
            toast.error(
                `Nisbat noto'g'ri! BN-5: ${bn5Percent.toFixed(1)}%, Mel: ${melPercent.toFixed(1)}%`
            );
            return false;
        }
    ...
    const renderInputFields = () => {
        const fields = [
            ...
            { label: 'Boshqa xarajatlar', key: 'extra', placeholder: '271000', readOnly: true },
                ...
                <div className="bitum-dialog-overlay">
                    <div className="bitum-dialog-box">
                        ...
                        <p>
                            ...
                            <strong>
                                {(
                                    (parseFloat(currentBn5Process.bn5Amount) + parseFloat(currentBn5Process.melAmount)) -
                                    inputValues.reduce((sum, input) => sum + (parseFloat(input.bn) || 0), 0)
                                ).toLocaleString()} kg
                            </strong>
                        </p>
                        ...
                        <div className="input-container">
                            ...
                            <input
                                type="text"
                                className="input-field"
                                placeholder={unitType === 'dona' ? "Qoplar sonini kiriting" : "Kraf qog'oz miqdorini (kg)"}
                                value={quantity}
                                onChange={(e) => setQuantity(e.target.value)}
                            />
                            <button className="action-button" onClick={handleOK}>OK</button>
                        </div>
                                ...
                                inputValues.map((input, index) => (
                                    <div key={index} className="input-value-card">
                                        ...
                                        <span className="package-info">
                                            {input.label === "Qop" ? input.label : "Kraf qog'oz"} {input.value} {input.label === "Qop" ? "dona" : "kg"}
                                        </span>
                                        {input.label === "Qop" && (
                                            <span className="package-info">
